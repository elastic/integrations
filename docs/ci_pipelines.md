...
On pull requests targeting a `backport-*` branch, an additional step (`check-changelog-versions-in-main`) verifies that no changelog version introduced by the PR already exists on `main`, catching sync collisions before merge. This step is coming with the auto-backport workflow (PR #20197).
