...
2. Prefer **manifest variable overrides** over hardcoding values in `stream.yml.hbs` (hardcoding causes Fleet UI values that have no effect).
...
| --- | --- | --- |
| **A — Integration-only** | Not in input package (e.g. `metrics_path`) | Add data stream var + reference in slim `stream.yml.hbs` |
...
| **C — Inherit** | Input default matches legacy (e.g. `use_types: true`) | Remove from local template and data stream manifest; do not redeclare |

For each **A** and **B** variable, also confirm:

- `show_user`: `true` (user-facing) or `false` (advanced/hidden)?
...
Present the variable matrix (name → category → default → `show_user`) and get confirmation before editing.
...
| Policy tests | Confirm default (`vars: ~`) + overrides test; which vars to exercise in overrides? |
...
- Per data stream: remove legacy `input:` key, `streams[].package`, `dataset:`, category A/B `streams[].vars` only, slim template contents
...
5. **Policy tests** — `test-default.yml`, `test-overrides.yml`; generate expectations only after developer approval
...
- Policy output: `data_stream.dataset` and `output_permissions` index names (e.g. `metrics-<dataset>-ep`)
...
Var only in integration template?       → A: add var + slim template reference
...
- Hardcoding overrides in `stream.yml.hbs` without developer acknowledgement → Fleet UI mismatch
