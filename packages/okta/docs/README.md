...
| okta.device.disk_encryption_type | The disk encryption type used on the device. | keyword |
...
| okta.device.screen_lock_type | The mechanism for locking the device's screen. | keyword |
| okta.device.secure_hardware_present | Whether there is secure hardware present on the device. | boolean |
