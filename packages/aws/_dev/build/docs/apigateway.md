...
To use these fields in the integration, create custom pipelines. These pipelines will process field names or values. You'll also need to create a custom mapping to store your data efficiently. For more information, refer to the [Tutorial: Transform data with custom ingest pipelines](https://www.elastic.co/guide/en/fleet/current/data-streams-pipeline-tutorial.html).

You can find more details in the [Logs reference](#logs-reference).
...
You can find more details in the [Metrics reference](#metrics-reference).
...
For more details, refer to the [AWS integration documentation](https://docs.elastic.co/integrations/aws#requirements).
...
> **Note:** Agentless deployment is currently supported for API Gateway metrics only. Log collection is not available via agentless and requires a traditional Elastic Agent deployment.
...
For step-by-step instructions on how to set up an integration, refer to the [Quickstart: Monitor hosts with Elastic Agent](docs-content://solutions/observability/get-started/quickstart-monitor-hosts-with-elastic-agent.md).
