...
| Assume Role with External ID | `role_arn`, `external_id` | [Use an IAM role with external ID](#use-an-iam-role-with-external-id) |
...
#### Use an IAM role with External ID

For advanced use cases, you can assume an IAM role in another account by providing both the `role_arn` and an `external_id`.
The external ID is an extra security measure required by the target account's trust policy; see [AWS documentation on external IDs](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user_externalid.html) for details.

Select the **Assume Role with External ID** method and provide both `role_arn` and `external_id`.
...
period. This will cause data points missing or none get collected by the agent. In this case, specify a
latency parameter so collection start time and end time will be shifted by the given latency amount.

To check how much the latency is, you can log into the AWS CloudWatch portal. Wait till a new point to
...
This is an overview of the type of data you can collect from each AWS service:
...
Visit the page for each individual AWS integration to get more details about exported fields.