...
- Collect metrics related to the cache, request, session and thread pool and collect logs related to access, catalina, and localhost.
...
Metrics give you insight into the statistics of the Apache Tomcat. The `Metric` data streams collected by the Apache Tomcat integration are `cache`, `request`, `session` and `thread pool`, so that the user can monitor and troubleshoot the performance of the Apache Tomcat instance.
