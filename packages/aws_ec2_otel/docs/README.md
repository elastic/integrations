...
| [AWS EC2 OTel] EBS attached volume status check failed | Alerts when an EBS volume attached to an EC2 instance fails its status check, indicating the volume is unreachable or unable to complete I/O operations. Distinct from the instance and system status checks. |
