...
3.  Enable timestamps for log messages to ensure Elastic Agent can parse the events:
    ...
    > **Important**: When using `show-timezone`, Cisco IOS includes a timezone abbreviation (such as `AEST` or `CST`) in the log message. The integration has a built-in map for common abbreviations (including SGT, ICT, MYT, PHT, NPT, TRT, BRT, ART, CLT, and PET), so these resolve automatically. For abbreviations not in the built-in map, you must configure the `Timezone Map` setting in the Kibana integration to map them to valid IANA timezones (for example, `AEST` to `Australia/Sydney`). Without this mapping, timestamps fall back to UTC or your configured `Timezone` offset. Documents with unresolved abbreviations are tagged `cisco_ios_unmapped_timezone` — use that tag to find affected events. Refer to the [Timezone configuration](#common-configuration-issues) section for more details.
...
- **Timezone mapping failures**: If your logs show an incorrect time that's offset by several hours, first check whether the timezone abbreviation is in the integration's built-in map (SGT, ICT, MYT, PHT, NPT, TRT, BRT, ART, CLT, PET). For abbreviations not in the built-in map, configure the `Timezone Map` setting to translate Cisco's short-form timezone strings (like `AEST`) to standard IANA formats. To find events affected by an unrecognised abbreviation, filter on `tags: cisco_ios_unmapped_timezone` in Kibana Discover — these events will have their timestamp parsed with the fallback offset instead of the device's actual timezone. Separately, if the `Timezone` setting itself is not a valid IANA timezone or offset (for example a mistyped `+25:00`), timestamps fall back to UTC and the affected events are tagged `cisco_ios_invalid_tz_offset` — filter on that tag to identify a misconfigured `Timezone` value.
