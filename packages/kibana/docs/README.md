...
An example event for `settings` looks as following:
...
```json
{
    ...
    "kibana": {
        ...
        "settings": {
            ...
            "uuid": "2076c0f0-fd6e-4ce0-bf04-59541bd3ba0c",
            "version": "7.10.0"
        }
    },
    "metricset": {
...
| data_stream.namespace | Data stream namespace. | constant_keyword |
| data_stream.type | Data stream type. | constant_keyword |
| kibana.settings.host | Kibana instance hostname | keyword |
| kibana.settings.index | Name of Kibana's internal index | keyword |
| kibana.settings.locale |  | keyword |
| kibana.settings.name | Kibana instance name | keyword |
| kibana.settings.port |  | integer |
| kibana.settings.snapshot | Whether the Kibana build is a snapshot build | boolean |
| kibana.settings.status | Kibana instance's health status | keyword |
| kibana.settings.transport_address | Kibana server's hostname and port | keyword |
| kibana.settings.uuid | Kibana instance UUID | keyword |
| kibana.settings.version | Kibana version | keyword |
