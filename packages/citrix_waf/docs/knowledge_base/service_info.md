...
- **Compliance and Auditing:** Maintain a detailed audit trail of all web requests and security events to meet regulatory requirements (for example, PCI-DSS, HIPAA) using structured CEF logs.
...
*   **Citrix WAF logs (using Syslog):** Collects Citrix WAF logs transmitted over the network using TCP or UDP. This includes real-time security violations and signature match events.
*   **Citrix WAF logs (using File):** Collects Citrix WAF logs from local files on the system where the Elastic Agent is running.
...
*   **Security Event Metadata:** Information regarding the specific security check triggered, the action taken (for example, block, log), and the severity of the event.
...
- **Integration Installation:** The Citrix WAF integration must be added to the Elastic Agent's policy using Kibana.
...
### Collecting logs from Citrix Web App Firewall using TCP
...
### Collecting logs from Citrix Web App Firewall using UDP
1. In Kibana, navigate to **Management > Integrations** and search for **Citrix Web App Firewall**.
2. Click **Add Citrix Web App Firewall**.
...
### Collecting logs from Citrix Web App Firewall using file
1. In Kibana, navigate to **Management > Integrations** and search for **Citrix Web App Firewall**.
2. Click **Add Citrix Web App Firewall**.
...
- **Simulate Policy Violation:** Attempt to access a sensitive path (for example, `curl http://<waf_app_ip>/etc/passwd`) to trigger a WAF block or log event.
...
- **Port Conflicts**: Ensure that the `Listen Port` configured in Kibana (for example, 9001) is not being used by another service on the Elastic Agent host. Use `netstat -ano` or `ss -tulpn` to verify the Agent is listening on the expected port.
