...
The dashboard reads metric-stream style OpenTelemetry documents from `metrics-aws.sqs.otel-*` and isolates CloudWatch statistics with `stat` before aggregating values.
