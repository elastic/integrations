...
| atlassian_cloud.audit.actor.app.id | App or Connect app ID, or system actor identifier (e.g., confluence). | keyword |
...
| atlassian_cloud.audit.actor.auth.auth_type | Authentication type for the actor (e.g., session, container-token). | keyword |
...
| atlassian_cloud.audit.location.geo | Coarse geo label from Atlassian (e.g., EU, NA). | keyword |
| atlassian_cloud.audit.message.format | Message format (e.g., simple). | keyword |
