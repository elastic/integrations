...
```json
{
    "@timestamp": "2026-06-26T06:28:25.073Z",
    "body": {
    ...
    "observed_timestamp": "2026-06-26T06:28:25.07368947Z",
    "organization": {
...
| agent_path_count | Number of agent paths in the plugin. | long |
| cache_creation_tokens | Number of tokens used for cache creation. | long |
| cache_read_tokens | Number of tokens served from cache. | long |
| command_name | Slash command name from user_prompt events. | keyword |
| command_path_count | Number of command paths in the plugin. | long |
| cost_usd | Cost of the API request in USD. | double |
| cost_usd_micros | Cost of the API request in millionths of a USD. | long |
| data_stream.dataset | Data stream dataset. | constant_keyword |
| data_stream.namespace | Data stream namespace. | constant_keyword |
| data_stream.type | Data stream type. | constant_keyword |
| decision | Permission decision (accept, reject). | keyword |
| decision_source | Source of the permission decision (config, user_temporary, user_permanent). | keyword |
| decision_type | Decision type (accept, reject). | keyword |
| duration_ms | Duration of the operation in milliseconds. | long |
| ecs.version | ECS version this event conforms to. `ecs.version` is a required field and must exist in all events. When querying across multiple indices -- which may conform to slightly different ECS versions -- this field lets integrations adjust to the schema version of the events. | keyword |
| effort | Thinking effort level (high, medium, low). | keyword |
| enabled_via | How the plugin was enabled (for example user-install). | keyword |
| error.message | Error message. | match_only_text |
| error_code | Error code (for example ENOENT). | keyword |
| error_detail | Error message from a failed operation. | keyword |
| error_type | Error classification (for example ShellError). | keyword |
...
| event_name | OTel event name identifying the event type. | keyword |
| file.path | Full path to the file, including the file name. It should include the drive letter, when appropriate. | keyword |
| file.path.text | Multi-field of `file.path`. | match_only_text |
| from_mode | Permission mode before the change. | keyword |
| has_hooks | Whether the session has registered hooks. | boolean |
| has_mcp | Whether the session has MCP servers. | boolean |
| hook_event | Hook event type (for example SessionStart, PreToolUse). | keyword |
| hook_matcher | Pattern the hook matches against (for example Bash). | keyword |
| hook_name | Name of the registered hook. | keyword |
| hook_source | Source of the hook definition (for example settings.json). | keyword |
| hook_type | Hook type (for example command). | keyword |
| host_owned_mcp | Whether MCP servers are host-owned ("true" or "false"). | boolean |
| input_tokens | Number of input tokens consumed. | long |
| invocation_trigger | How the skill was invoked (for example user-slash). | keyword |
| is_plugin | Whether the MCP server is a plugin. | boolean |
| managed_only | Whether the MCP server is managed-only ("true" or "false"). | keyword |
| marketplace.name | Name of the marketplace entry. | keyword |
| mcp_server_name | Name of the MCP server. Extracted from tool_parameters_flattened by the ingest pipeline. | keyword |
| mcp_server_scope | MCP server scope from tool_parameters. | keyword |
| mcp_tool_name | Name of the MCP tool invoked. Extracted from tool_parameters_flattened by the ingest pipeline. | keyword |
| model | AI model used for the request (for example claude-sonnet-4-6). | keyword |
| num_blocking | Number of hooks that blocked execution. | long |
| num_cancelled | Number of hooks that were cancelled. | long |
| num_hooks | Number of hooks registered for this event. | long |
| num_non_blocking_error | Number of hooks that failed without blocking. | long |
| num_success | Number of hooks that completed successfully. | long |
| organization.id | Anthropic organization identifier. | keyword |
| output_tokens | Number of output tokens generated. | long |
| plugin.name | Plugin name. | keyword |
| plugin.scope | Plugin scope (for example project, user). | keyword |
| plugin.version | Plugin version. | keyword |
| plugin_id_hash | Hash of the plugin identifier. | keyword |
| process.command_line | Full command line that started the process, including the absolute path to the executable, and all arguments. Some arguments may be filtered to protect sensitive information. | wildcard |
| process.command_line.text | Multi-field of `process.command_line`. | match_only_text |
| prompt.id | Prompt turn identifier within a session. | keyword |
| prompt_length | Length of the user prompt in characters. | long |
| prompt_text | User prompt text. Only present when OTEL_LOG_USER_PROMPTS is enabled. Renamed from OTel attribute 'prompt' to avoid conflict with prompt.id path. | text |
| query_source | Source of the API query (repl_main_thread, generate_session_title, tool_feedback). | keyword |
| related.hosts | All hostnames or other host identifiers seen on your event. Example identifiers include FQDNs, domain names, workstation names, or aliases. | keyword |
| related.user | All the user names or other user identifiers seen on the event. | keyword |
| request_id | Anthropic API request identifier. | keyword |
| safe_mode | Whether safe mode is active ("true" or "false"). | keyword |
| server_name | MCP server name from connection events. | keyword |
| server_scope | MCP server scope (for example project, user). | keyword |
| session.id | Claude Code session identifier. | keyword |
| skill.name | Activated skill name. | keyword |
| skill.source | Source of the skill definition. | keyword |
| skill_path_count | Number of skill paths in the plugin. | long |
| source | Source of the event (for example user, system). | keyword |
| speed | Request speed tier (normal, fast). | keyword |
| status | Connection status (for example connected, failed). | keyword |
| success | Whether the tool call succeeded ("true" or "false"). | keyword |
| terminal.type | Terminal emulator type. | keyword |
| to_mode | Permission mode after the change. | keyword |
| tool_input | JSON-encoded tool input. Only present when OTEL_LOG_TOOL_DETAILS is enabled. | text |
| tool_input_flattened | Parsed tool_input as a flattened object for structured queries. Populated by the ingest pipeline. | flattened |
| tool_input_size_bytes | Size of tool input in bytes. | long |
| tool_name | Name of the tool invoked (for example Bash, Read, Write, mcp_tool). | keyword |
| tool_parameters | JSON-encoded tool parameters. Only present when OTEL_LOG_TOOL_DETAILS is enabled. | text |
| tool_parameters_flattened | Parsed tool_parameters as a flattened object for structured queries. Populated by the ingest pipeline. | flattened |
| tool_result_size_bytes | Size of tool result in bytes. | long |
| tool_use_id | Unique identifier for this tool invocation. | keyword |
| total_duration_ms | Total duration of all hook executions in milliseconds. | long |
| transport_type | MCP transport type (for example stdio, sse). | keyword |
| trigger | What triggered the permission mode change. | keyword |
| url.full | If full URLs are important to your use case, they should be stored in `url.full`, whether this field is reconstructed or present in the event source. | wildcard |
| url.full.text | Multi-field of `url.full`. | match_only_text |
| user.account_id | Anthropic account identifier. | keyword |
| user.account_uuid | Anthropic account UUID. | keyword |
