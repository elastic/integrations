...
- The default `End Interval` is configured to `24h`, you can however change these to your required values. The `End Interval` was introduced to allow a sliding 
  window to exist in combination with the `Initial Interval`. If you do not require a sliding window you can set this to `0s` which will cause the `End Interval` to 
