## Kubernetes Pod TX Error Rate alert

In order to install the Pod TX Error Rate alert run the following:
```bash
curl -X PUT "https://elastic:changeme@localhost:9200/_watcher/watch/Pod-TX-Error-Rate?pretty" -k -H 'Content-Type: application/json' -d'
{
  "trigger": {
    "schedule": {
      "interval": "10m"
