...
This integration is for M365 Defender logs, previously known as Microsoft Threat Protection.

## Configuration

To configure access for Elastic Agent to communicate with Microsoft 365 Defender, you will have to create a new Azure Active Directory Application registration. This will return OAuth tokens with access to the Microsoft 365 Defender API.

The procedure to create an Azure Active Directory application is found on the below link:

[Create an app to access Microsoft 365 Defender without a user](https://docs.microsoft.com/en-us/microsoft-365/security/mtp/api-create-app-web?view=o365-worldwide#create-an-app)

When giving the application the API permissions described in the documentation (Incident.Read.All) it will only grant access to read Incidents from Microsoft 365 Defender and nothing else in the Azure Active Directory domain.
...
> **_NOTE:_** If using the Microsoft 365 Defender API in a US GCC or GCC-High environment, you will need to change the default values for OAuth Server URL and API URL Endpoint in your Elastic Agent configuration.
> - OAuth Server URL: `https://logon.microsoftonline.us`
> - API URL Endpoint: `https://api-gov.security.microsoft.us`


After connecting, an example event for `log` looks as following:
