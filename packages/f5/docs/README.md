...
- `ltm` dataset: supports Big-IP LTM logs exported via iRule.
...
The `ltm` dataset collects Big-IP LTM logs exported via iRule.
Add this iRule under Virtual Server -> iRules and add a Pool with your destination IP(s) and Port as Member(s).
...
when RULE_INIT  {
    ...
    set static::facility "<133>"
    set static::msg_prefix "<133>1"
}

when CLIENT_ACCEPTED priority 100 {
    set tcp_start_time [clock clicks -milliseconds]
    set hslHTTP [HSL::open -proto UDP -pool /Common/pool_syslog]
    #set hslHTTP [HSL::open -publisher /Common/PoolSyslog]
    set logMsgHttp ""

    set logMsgTCP "srcip=\"[IP::client_addr]\""
    set http_req_time $tcp_start_time
}

when CLIENT_CLOSED priority 800 {
    unset logMsgHttp
    unset logMsgTCP
}

when HTTP_REQUEST priority 720 {
    set http_req_time [clock clicks -milliseconds]
    set logMsgHttp "reqtime=\"${http_req_time}\""
    if { [HTTP::has_responded] } {
        # Exit if there was a previous HTTP::(respond|redirect|retry)
        return
    }

    append logMsgHttp " method=\"[HTTP::method]\" host=\"[HTTP::host]\" uri=\"[HTTP::uri]\" uagent=\"[HTTP::header value "user-agent"]\" vsip=\"[IP::local_addr]\" vsname=\"[virtual name]\" vs=\"[LB::server addr]\""
}

when HTTP_RESPONSE {
    set RESPONSE_MSECS [expr {[clock clicks -milliseconds] - $http_req_time}]

    append logMsgHttp " status=\"[HTTP::status]\" statusmsg=\"[HTTP::status]\" responsetime=\"${RESPONSE_MSECS}\" bytes=\"0\" realserver=\"[IP::server_addr]\" realport=\"[TCP::server_port]\" pool=\"[LB::server pool]\""
}

when HTTP_RESPONSE_RELEASE {
    set http_resp_time [clock clicks -milliseconds]

    ###array set timestamp [clock fields [clock milliseconds] -resolution milliseconds]
    ###set strTS "$timestamp(year)-$timestamp(month)-$timestamp(day)T$timestamp(hour):$timestamp(min):$timestamp(sec).$timestamp(millisec)+02:00"
    set strTS [format "%s.%03d" [clock format [expr {$http_req_time / 1000}] -format "%Y-%m-%dT%H:%M:%S"]  [expr {$http_req_time % 1000}] ]
    #set strTS [clock format [clock seconds] -format "%Y-%m-%dT%H:%M:%S.000"]

    HSL::send $hslHTTP "${static::msg_prefix} ${strTS}Z ${static::bigip} f5LTM - ID2 \[$logMsgTCP $logMsgHttp rtt=\"[expr {$http_resp_time - $http_req_time}]\" sessiontime=\"[expr {$http_resp_time - $tcp_start_time}]\"\]\n"
    #log local0. "${static::msg_prefix} ${strTS}Z ${static::bigip} f5LTM3 3 \[$logMsgTCP $logMsgHttp rtt=\"[expr {$http_resp_time - $http_req_time}]\" sessiontime=\"[expr {$http_resp_time - $tcp_start_time}]\"\]"
}
```
