...
```json
{
    ...
    "agent": {
        "ephemeral_id": "c1a8aee3-3caa-472e-89a0-866b421872f3",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
    ...
    "event": {
        ...
        "created": "2022-03-29T13:20:53.645Z",
        "dataset": "mimecast.audit_events",
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "6faaca1c-ab8c-4c7b-8957-6609d0cda440",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "b2990c71-8649-46d7-b0cb-95cbc3fcd08c",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "d9535010-2a7e-4f53-8121-3197e30375a9",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "64fc0f24-0f73-489b-97ce-b7f0d235077c",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
    ...
    "email": {
        "attachments": {
            ...
            "hash": "cabd7cb6e1822fd9e1fc9bcf144ee26ee6bfc855c4574ca967dd53dcc36a1254"
        },
        "direction": "inbound",
        "from": {
            "address": "\u003c\u003e"
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "42007e44-664e-4de7-a3b9-ba233a686ed8",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
...
| email.message_id | Identifier from the RFC5322 Message-ID - header field that refers to a particular version of a particular message. | wildcard |
| email.subject | A brief summary of the topic of the message | keyword |
| email.to.address | Email of the recipient/user who click the link. | keyword |
| event.action | The action captured by the event. This describes the information in the event. It is more specific than `event.category`. Examples are `group-add`, `process-started`, `file-created`. The value is normally defined by the implementer. | keyword |
| event.dataset | Event dataset | constant_keyword |
| event.module | Event module | constant_keyword |
| event.original | Raw text message of entire event. Used to demonstrate log integrity or where the full log message (before splitting it up in multiple parts) may be required, e.g. for reindex. This field is not indexed and doc_values are disabled. It cannot be searched, but it can be retrieved from `_source`. If users wish to override this and index this field, please see `Field data types` in the `Elasticsearch Reference`. | keyword |
...
```json
{
    ...
    "agent": {
        "ephemeral_id": "1fc6c283-31b9-46a7-86af-289c1e0abbe0",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
    ...
    "event": {
        "agent_id_status": "verified",
        "category": "threat",
        "created": "2022-03-29T13:23:15.543Z",
        "dataset": "mimecast.threat_intel_malware_customer",
        "ingested": "2022-03-29T13:23:16Z",
        "kind": "enrichment",
...
```json
{
    "@timestamp": "2021-11-19T01:28:37.099Z",
    "agent": {
        "ephemeral_id": "c0b0807d-c1dc-4775-8d0f-c2e12343b3ef",
        "hostname": "docker-fleet-agent",
        "id": "89305efe-fd22-4f19-9f55-66bbeefccd60",
        "name": "docker-fleet-agent",
        "type": "filebeat",
    ...
    "event": {
        "agent_id_status": "verified",
        "category": "threat",
        "created": "2022-03-29T13:24:10.977Z",
        "dataset": "mimecast.threat_intel_malware_grid",
        "ingested": "2022-03-29T13:24:11Z",
        "kind": "enrichment",
        "original": "{\"created\":\"2021-11-19T01:28:37.099Z\",\"id\":\"indicator--456ac916-4c4e-43be-b7a9-6678f6a845cd\",\"labels\":[\"malicious-activity\"],\"modified\":\"2021-11-19T01:28:37.099Z\",\"pattern\":\"[file:hashes.'SHA-256' = 'ec5a6c52acdc187fc6c1187f14cd685c686c2b283503a023c4a9d3a977b491be']\",\"type\":\"indicator\",\"valid_from\":\"2021-11-19T01:28:37.099Z\"}",
        "type": "indicator"
    },
