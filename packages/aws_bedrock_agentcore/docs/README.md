...
Data streams:
 - `metrics`: Collects Amazon Bedrock AgentCore runtime metrics including invocations, sessions, latency, performance indicators, error rates, throttling metrics, resource utilization (CPU and memory), token counts, target execution metrics, and authorization metrics.
