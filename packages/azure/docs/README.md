...
_string_
Optional. By default, the name of the storage account container that the state/offsets will be stored and updated, is generated by the concatenation of agent, logs type and the value of `eventhub`. This may fail if it produces a not valid name for a storage account container. In this case it's possible to override the generated value with the one set manually in this setting.
