...
if [ "$JOLOKIA_ENABLED" = 'yes' ]; then
    ...
    echo "export YARN_NODEMANAGER_OPTS="-javaagent\:`echo /jolokia-jvm-${JOLOKIA_VERSION}-agent.jar=host=${JOLOKIA_HOST},port=7782`"" >> "${HADOOP_HOME}/etc/hadoop/hadoop-env.sh"
    echo "export YARN_RESOURCEMANAGER_OPTS="-javaagent\:`echo /jolokia-jvm-${JOLOKIA_VERSION}-agent.jar=host=${JOLOKIA_HOST},port=7781`"" >> "${HADOOP_HOME}/etc/hadoop/hadoop-env.sh"
fi
