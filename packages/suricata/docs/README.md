...
This integration collects events from Suricata's EVE JSON log. The event types are configurable within Suricata but typically include:

- `alert`: Security alerts generated by rules.
- `flow`: Network flow records.
- `dns`, `http`, `smb`, `tcp`, `tls`, `ssh`, `smtp`: Protocol-specific transaction logs.
