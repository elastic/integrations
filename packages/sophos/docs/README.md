...
This integration collects logs from Sophos firewalls by receiving syslog data over TCP or UDP, or by reading directly from log files. It captures comprehensive firewall telemetry including security heartbeats, system events, traffic logs, and specific log categories such as DNS, DHCP, HTTP, and Packet Filter logs.
...
Integrating Sophos logs with Elastic provides a powerful solution for enhancing security posture and operational visibility. Key use cases include:
*   Real-time threat detection: Leverage Elastic SIEM to detect and respond to threats identified in firewall logs, including IPS and ATP events.
...
Select **Collecting syslog from Sophos via UDP** and configure the following settings:
...
Select **Collecting syslog from Sophos via TCP** and configure the following settings:
...
Select **Collecting syslog from Sophos via file** and configure the following settings:
...
The `xg` data stream collects Sophos XG logs.
...
- [Sophos UTM Documentation](https://docs.sophos.com/nsg/sophos-utm/) - General documentation hub for UTM devices.
