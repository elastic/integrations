...
### Supported channels
The receiver monitors Windows Event Log channels. Typical channels include:
- Application
- Security
- System
- Microsoft-Windows-PowerShell/Operational

You can list available channels on a Windows host by running `Get-WinEvent -ListLog * | Format-List -Property LogName` in PowerShell.

### Platform requirement
The Windows Event Log receiver runs only on Windows. The collector must be deployed on a Windows host to collect event logs.
