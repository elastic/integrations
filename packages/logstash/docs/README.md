...
| logstash.node.address |  | keyword |  |
| logstash.node.name |  | keyword |  |
| logstash.node.status |  | keyword |  |
| logstash.node.symptom |  | keyword |  |
| logstash.node.symptom.text | Multi-field of `logstash.node.symptom`. | match_only_text |  |
| logstash.node.uuid |  | keyword |  |
| logstash.node.version |  | keyword |  |
...
| logstash.node.stats.jvm.mem.heap_max_in_bytes |  | long | gauge |
...
| logstash.node.stats.jvm.threads.count | current number of threads | long | gauge |
| logstash.node.stats.jvm.threads.peak_count | peak number of threads | long | gauge |
...
| logstash.node.stats.queue.events_count |  | long | gauge |
...
| logstash.pipeline.info.batch_delay | Batch delay for the running pipeline | long |  | gauge |
| logstash.pipeline.info.batch_size | Batch size for the running pipeline | long |  | gauge |
...
| logstash.pipeline.total.queues.events | Number of events in the PQ for this pipeline | long |  | gauge |
...
| logstash.pipeline.plugin.input.metrics.beats.current_connections | Current connections count | long |  | gauge |
| logstash.pipeline.plugin.input.metrics.beats.peak_connections | Number of peak connections | long |  | gauge |
