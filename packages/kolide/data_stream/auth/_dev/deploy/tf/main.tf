...
resource "aws_s3_object" "object" {
  ...
  bucket = aws_s3_bucket.kolide.id
  # Flat fixture files are named "<request-id>__<event-type>-<timestamp>.json";
  # reconstruct the real Kolide Log Pipeline key layout:
  # kolide/auth_logs/<request-id>/<event-type>-<timestamp>.json
  key    = "kolide/auth_logs/${replace(each.value, "__", "/")}"
