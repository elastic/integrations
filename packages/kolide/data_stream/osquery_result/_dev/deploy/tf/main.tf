...
resource "aws_s3_bucket" "kolide" {
  bucket = "elastic-package-kolide-osquery-result-bucket-${var.TEST_RUN_ID}"
}

resource "aws_sqs_queue" "kolide_queue" {
  name   = "elastic-package-kolide-osquery-result-queue-${var.TEST_RUN_ID}"
  policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
    {
      ...
      "Resource": "arn:aws:sqs:*:*:elastic-package-kolide-osquery-result-queue-${var.TEST_RUN_ID}",
      "Condition": {
...
resource "aws_s3_object" "object" {
  ...
  bucket = aws_s3_bucket.kolide.id
  # Flat fixture files are named "<action>__<timestamp>.json"; reconstruct a
  # Kolide Log Pipeline-shaped key layout: kolide/results/<action>/<timestamp>.json
  key    = "kolide/results/${replace(each.value, "__", "/")}"
