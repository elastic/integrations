...
The Fortinet FortiManager Logs integration collects log messages pushed via `tcp` or `udp`, or read from active log files using the `filestream` input. The integration parses Syslog-formatted data from the following functional areas:
*   System Manager (`system`) logs: System-level events, process monitoring, and general health status.
*   FortiGuard Service (`fgd`) logs: Updates and communications related to FortiGuard security services.
*   Security Console (`scply`) logs: Records of security policy changes and management activities.
*   FortiGate-FortiManager Protocol (`fgfm`) logs: Status updates for communication between the manager and managed devices.
*   Device Manager (`devmgr/dvm`) and Deployment Manager (`dm`) logs: Information about device registration and configuration deployment status.
*   Object Changes (`objcfg`) and Script Manager (`scrmgr`) logs: Detailed auditing of configuration modifications and script executions.
*   System maintenance: Logs from the Firmware Manager (`fmwmgr`), Log Daemon (`logd`), and Debug IO (`iolog`).
*   Operational reporting: Logs related to log files, logging status, and reports generated by the FortiManager or FortiAnalyzer features.
...
- Elastic Stack version `8.x` or newer.
...
Elastic Agent is required to stream data from the syslog or log file receiver and ship the data to Elastic, where the events will be processed via the integration's ingest pipelines.
...
| Setting | Description |
|---|---|
| **Paths** | A list of glob-based paths to monitor (e.g., `/var/log/fortinet/fortimanager.log*`). |
| **Timezone Offset** | Specify an IANA timezone or offset (e.g., `Europe/Amsterdam` or `-05:00`) for logs with no timezone information. Default is `local`. |
...
| Setting | Description |
|---|---|
| **Listen Address** | The bind address for the TCP listener (e.g., `localhost` or `0.0.0.0`). |
| **Listen Port** | The TCP port number to listen on (e.g., `9022`). |
| **Preserve original event** | If checked, a raw copy of the original log is stored in the `event.original` field. |
...
| Setting | Description |
|---|---|
| **Listen Address** | The bind address for the UDP listener (e.g., `localhost` or `0.0.0.0`). |
| **Listen Port** | The UDP port number to listen on (e.g., `9022`). |
| **Preserve original event** | If checked, a raw copy of the original log is stored in the `event.original` field. |
...
2.  Trigger data flow on the FortiManager device by performing one of the following actions:
    *   **Generate a configuration event**: Make a minor change in the GUI (e.g., adding a comment) and save it.
...
5.  Verify that logs appear in the results. Expand an entry and confirm the following fields are populated:
    ...
    *   `fortimanager.log.type` (e.g., `event` or `traffic`)
    *   `fortimanager.log.subtype` (e.g., `system`, `objcfg`, or `devmgr`)
...
For more information about configuring and troubleshooting FortiManager logs, refer to the following resources:
...
- Elastic Agent scaling: A single Elastic Agent can handle a significant volume of logs, but for very high-throughput environments, you'll want to consider deploying multiple Elastic Agents. Each agent can be configured to listen on a dedicated port or monitor specific subsets of log files, which distributes the ingestion load. Ensure your agents are adequately resourced with CPU, memory, and disk I/O, and are placed strategically (for example, on the same network segment) to minimize network latency to the FortiManager or FortiAnalyzer devices.
