...
Use the F5 BIG-IP integration to collect and parse data from F5 BIG-IP using **telemetry streaming** and then visualize that data in Kibana. For **LTM, AFM, APM, ASM (WAF), AVR, System Information, and iHealth Information**, the integration expects **JSON** telemetry. **Bot Defense and DoS** events are not parsed through that JSON category path: they must appear in the raw log as **key=value** fields. Ingesting or parsing the **syslog events** produced at the operating system level is not supported. Use the [Custom Logs integration](https://docs.elastic.co/integrations/log) to ingest these system-level log messages instead.
...
**Logs** help users to keep a record of events happening on the network using telemetry streaming.
The log data stream collected by the F5 BIG-IP integration includes events that are related to network traffic. See more details in the [logs examples documentation](https://clouddocs.f5.com/products/extensions/f5-telemetry-streaming/latest/output-example.html). Most routed log types use the `telemetryEventCategory` field in JSON; Bot Defense and DoS use the **key=value** log format described above instead of that category.

This integration targets the event types mentioned below:
...
- **ASM** is F5's web application firewall (WAF) solution. It allows users to tailor acceptable and expected application behavior on a per-application basis. For more information, refer to the link [here](https://www.f5.com/pdf/products/big-ip-application-security-manager-overview.pdf). **ASM request/security events** in this integration are JSON telemetry with `telemetryEventCategory` set to **ASM**.
- **BOT and DOS** (Bot Defense and distributed DoS) are supported when the forwarded message uses the **key=value** logging shape the ingest pipeline expects (`device_product="Application Security Module"` with comma-separated fields for many Bot events, or `device_product=ASM` with semicolon-separated fields for many DoS events). That is different from JSON routed only by `telemetryEventCategory`. For more information on BOT, refer to the knowledge base article [here](https://my.f5.com/manage/s/article/K17680287). For more information on DOS, refer to the article [here](https://my.f5.com/manage/s/article/K75699030).
