...
Join keys must be single-valued: a report can list several CVE identifiers, so `hackerone.report.attributes.cve_ids` never matches and adds only `null` columns.
