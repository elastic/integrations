...
- `alert` dataset: It is typically used to retrieve detailed information about specific security alerts generated by the CrowdStrike Falcon platform, via Falcon's Alert API. It allows users to programmatically access information about threats, incidents and responses.

- `host` dataset: It retrieves all the hosts in your environment providing information such as device metadata, configuration, and status. It is more focused to provide the management and monitoring information of devices such as login details, status, policies, configuration etc.
