...
- **Incorrect timestamps**: A time zone in the log header takes precedence over the **Timezone** setting, which applies only to messages that carry no time zone. `event.timezone` records the zone each timestamp was interpreted in. Some abbreviations cannot be resolved automatically: the WLC truncates longer names to seven characters (for example `Singapo`), some have no equivalent zone (`SGT`, `ART`), and some are ambiguous (`CST` is both China and US Central). Use **Time Zone Map** to resolve these. Because an abbreviation is resolved to a region rather than a fixed offset, a WLC configured with a fixed zone and no summer time (for example `clock timezone CET 1`) is interpreted as `+02:00` during summer; map it to `+01:00` with **Time Zone Map** to pin the offset.
