...
5.  Save the configuration to ensure settings persist after a reboot:
`save config`
...
| **Timezone Map**            | Maps time zone abbreviations in the log header (`tz_short`) to an IANA time zone or fixed offset (`tz_long`), for example `CEST` to `Europe/Paris`.                              |
...
- **Incorrect timestamps**: A time zone in the log header takes precedence over the **Timezone** setting, which applies only to messages that carry no time zone. Some abbreviations cannot be resolved automatically: the WLC truncates longer names to seven characters (for example `Singapo`), some have no equivalent zone (`SGT`, `ART`), and some are ambiguous (`CST` is both China and US Central). Use **Timezone Map** to resolve these. Because an abbreviation is resolved to a region rather than a fixed offset, a WLC configured with a fixed zone and no summer time (for example `clock timezone CET 1`) is interpreted as `+02:00` during summer; map it to `+01:00` with **Timezone Map** to pin the offset.
