...
This integration has been tested with Vercel Drains delivering to the Vercel path (`/vercel`) of the Elastic Cloud managed endpoint.
...
> **Note**: The managed endpoint URL for Vercel is always your Elastic Cloud public endpoint plus `/vercel`. Do not point the drain at a self-hosted collector; the supported path is the Elastic Cloud managed endpoint.

### Step 4: Validate that data is arriving

In Kibana, open **Discover** and query the data streams listed in [Reference](#reference), for example `logs-vercel.logs.otel-*`. Vercel delivers drain payloads in batches, so allow a few minutes after saving a drain and generating traffic before the first documents appear.
