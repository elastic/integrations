...
* The transform reads the monitoring data from the backing indices of the `metrics-elasticsearch.stack_monitoring.index` data stream, and not from the data stream itself. If your ILM policy downsamples that data, the downsampled indices are skipped: downsampling turns the gauge fields into `aggregate_metric_double`, which the `top_metrics` aggregations used by this transform do not support. The same applies to backing indices renamed by a shrink action or mounted as searchable snapshots. Only data that is still held in a `.ds-` backing index is aggregated.
