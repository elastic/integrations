...
An example event for `security_stats` looks as following:
...
| elasticsearch.node.name | Node name. | keyword |  |
| elasticsearch.node.roles | Node roles. | keyword |  |
| elasticsearch.node.version | Elasticsearch version reported by the node (for example, `9.2.0`). | keyword |  |
| elasticsearch.security.stats.dls.cache.entries.count | Current number of cached entries on this node. | long | gauge |
| elasticsearch.security.stats.dls.cache.evictions.count | Number of entries evicted (due to size limit or expiration) since node startup. | long | counter |
| elasticsearch.security.stats.dls.cache.hits.count | Number of cache lookups served from the cache since node startup. | long | counter |
| elasticsearch.security.stats.dls.cache.hits.time.ms | Cumulative time spent serving cache hits since node startup, in milliseconds. | long | counter |
| elasticsearch.security.stats.dls.cache.memory.bytes | Current memory consumed by the DLS cache on this node, in bytes. | long | gauge |
| elasticsearch.security.stats.dls.cache.misses.count | Number of cache lookups that had to materialize an entry since node startup. | long | counter |
| elasticsearch.security.stats.dls.cache.misses.time.ms | Cumulative time spent materializing entries on cache misses since node startup, in milliseconds. | long | counter |
| error.message | Error message. | match_only_text |  |
| event.dataset | Name of the dataset. If an event source publishes more than one type of log or events (e.g. access log, error log), the dataset is used to specify which one the event comes from. It's recommended but not required to start the dataset name with the module name, followed by a dot, then the dataset name. | keyword |  |
| event.duration | Duration of the event in nanoseconds. If event.start and event.end are known this value should be the difference between the end and start time. | long |  |
| event.module | Name of the module this data is coming from. If your monitoring agent supports the concept of modules or plugins to process events of a given source (e.g. Apache logs), `event.module` should contain the name of this module. | keyword |  |
...
* The transform job will process all compatible historical data that is still on the hot or warm tier (see the note on cold and frozen tiers below), which has two implications: 1. if you have pre-8.17.1 data, this will not get picked up by the job and 2. it might take time for "live" data to be available, as the transform job works its way through all documents. You can modify the transform job as you please if need be.
...
* The transform reads the monitoring data from the backing indices of the `metrics-elasticsearch.stack_monitoring.index` data stream, and not from the data stream itself. If your ILM policy downsamples that data, the downsampled indices are skipped, as the transform cannot aggregate downsampled data. The same applies to indices renamed by a shrink action or mounted as searchable snapshots.
* The transform skips monitoring documents that have aged into the cold or frozen tiers. Reading them makes the transform checkpoint stall and stop advancing on deployments with a long monitoring retention. The transform only needs recent data (10 minute buckets, 60 second sync delay), so data that has aged out of hot or warm was already processed. If you need to backfill `monitoring-indices` from older data, clone the transform, remove the `_tier` `must_not` clause from its source query, and run the copy as a batch transform.
