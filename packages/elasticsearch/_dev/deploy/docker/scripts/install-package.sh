...
package_version=$(sed -nr 's/^version: (.*)$/\1/p' < "manifest.yml")
...
package_policy_file=_dev/deploy/docker/scripts/package-policy.json
...
policy=$(
  sed "s~__PACKAGE_VERSION__~$package_version~g" < "$package_policy_file" |
