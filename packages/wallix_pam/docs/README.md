...
| wallix_pam.begin | Raw start time extracted from the Wallix PAM event. | keyword |
| wallix_pam.by_uid | UID of the user that initiated the PAM session change. | long |
| wallix_pam.channel_name | RDP dynamic channel name. | keyword |
| wallix_pam.checkout_target | Time in seconds spent checking out a target. | double |
| wallix_pam.class_name | Window class name reported by the RDP proxy. | keyword |
| wallix_pam.client_name | Kerberos client principal name. | keyword |
| wallix_pam.data | Free-form Wallix PAM payload data. | wildcard |
| wallix_pam.description | Descriptive text emitted by Wallix PAM. | wildcard |
| wallix_pam.device | Device or host value reported by the Wallix PAM event. | keyword |
| wallix_pam.diagnostic | Parsed diagnostic message from a Wallix authentication event. | wildcard |
| wallix_pam.display_name | Display name reported by Wallix PAM. | keyword |
| wallix_pam.duration | Raw Wallix PAM duration string. | keyword |
| wallix_pam.encryption_type | Encryption type reported by Wallix PAM. | keyword |
| wallix_pam.end | Raw end time extracted from the Wallix PAM event. | keyword |
| wallix_pam.end_time | Raw Kerberos ticket end time. | keyword |
| wallix_pam.fetch_rights | Time in seconds spent fetching rights. | double |
| wallix_pam.files | Raw file list reported by Wallix session integrity. | wildcard |
| wallix_pam.flags | Flags emitted by Wallix PAM. | keyword |
| wallix_pam.identifier | Identifier associated with the Wallix PAM event. | keyword |
| wallix_pam.infos | Free-form details reported by Wallix PAM. | wildcard |
| wallix_pam.method | Authentication or access method reported by Wallix PAM. | keyword |
| wallix_pam.module | PAM module name. | keyword |
| wallix_pam.object | Object referenced by a Wallix audit event. | wildcard |
| wallix_pam.pam_state | PAM session state reported by Wallix PAM. | keyword |
| wallix_pam.primary_authentication | Time in seconds spent on primary authentication. | double |
| wallix_pam.primary_connection | Time in seconds spent establishing the primary connection. | double |
| wallix_pam.psid | Wallix PAM proxy session identifier. | keyword |
| wallix_pam.renew_time | Raw Kerberos ticket renew time. | keyword |
| wallix_pam.result | Result text emitted by Wallix PAM. | wildcard |
| wallix_pam.server_name | Kerberos server principal name. | keyword |
| wallix_pam.service | Service reported by Wallix PAM. | keyword |
| wallix_pam.session | Indicates whether a vault operation is session-based. | boolean |
| wallix_pam.session_id | Wallix PAM session identifier. | keyword |
| wallix_pam.session_uid | Session UID used by Wallix session integrity events. | keyword |
| wallix_pam.source | Event source value reported by Wallix PAM. | keyword |
| wallix_pam.start_time | Raw Kerberos ticket start time. | keyword |
| wallix_pam.status | Original Wallix PAM status value. | keyword |
| wallix_pam.subcomponent | Source Wallix PAM subcomponent extracted from the syslog header. | keyword |
| wallix_pam.target | Target value reported by Wallix PAM. | wildcard |
| wallix_pam.target_connection | Time in seconds spent establishing the target connection. | double |
| wallix_pam.text | Free-form text reported by Wallix PAM. | wildcard |
| wallix_pam.total | Total time in seconds reported by Wallix PAM. | double |
| wallix_pam.tty | TTY value reported by Wallix PAM. | keyword |
| wallix_pam.type | Original Wallix PAM event type. | keyword |
| wallix_pam.uid | UID reported by PAM. | long |
| wallix_pam.vault | Vault name referenced by the event. | keyword |
| wallix_pam.window | Window title reported by the RDP proxy. | keyword |


#### bastion sample event

An example event for `bastion` looks as following:
...
```json
{
    ...
    "event": {
        ...
        "kind": "event",
        "original": "<14>2026-04-24T11:16:09+02:00 srv_wallix rdpproxy[658172]: [RDP Session] session_id=\"14778a68c391e847438224a99db1\" client_ip=\"192.0.2.1\" target_ip=\"192.0.2.1\" user=\"user.name@example.org\" device=\"SrvName\" service=\"RDP\" account=\"username@platform.example.org\" type=\"KBD_INPUT\" data=\"whis/<backspace>/<backspace>os/<backspace>is\"",
        "type": [
            "info"
        ]
    },
    ...
    "user": {
        ...
        "target": {
            "name": "username@platform.example.org"
        }
    },
    "wallix_pam": {
        "data": "whis/<backspace>/<backspace>os/<backspace>is",
        "device": "SrvName",
        "session_id": "14778a68c391e847438224a99db1",
        "type": "KBD_INPUT"
    }
}
```
