...
## HTTPJSON vs CEL 
As of version 2.0.0 a new input type, the CEL input has been introduced. This input is currently marked as [Beta] while the older HTTPJSON input has been
marked as [Legacy]. The HTTPJSON input will not be iterated upon in the future although we will continue to provide support for it. The CEL input will be our
main focus of development going forward. You should only use either one as they provide similar functionality.

## Note (Important)
1. When it comes to HTTPJSON vs CEL, you should always use either one of them, having both enabled simultaneously can cause unexpected/duplicated results, as they operate on the same data streams.

2. When using the HTTPJSON input, use either the old alert data stream or the new [Beta] alert_v7 data stream. Do not have these two data streams enabled simultaneously. This only applies when using the HTTPJSON input as the CEL input only supports the latest alert_v7 data stream.
