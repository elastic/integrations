...
| mssql.metrics.pvs_pages_allocated | Total size in bytes currently allocated to the Persistent Version Store (PVS) for Accelerated Database Recovery (ADR). Derived from pvs_pages_allocated in sys.dm_tran_persistent_version_store_stats (page count × 8192). Available on SQL Server 2022 (16.x) and later. | long | byte | gauge |
