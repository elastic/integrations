...
- `log` data stream: This collects Stormshield logs via the `UDP` or `TCP` protocols.
...
Elastic Agent is required to stream data from the syslog or log file receiver and ship the data to Elastic, where the events'll be processed via the integration's ingest pipelines.
...
You'll see a sample event for the `log` data stream below:
