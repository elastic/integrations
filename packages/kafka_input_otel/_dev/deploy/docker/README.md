...
- **otelcol**: OpenTelemetry Collector Contrib with OTLP receiver, batch processor, and dual Kafka exporter. Receives from the generator and writes to six Kafka topics: `otlp_logs`, `otlp_metrics`, `otlp_spans` (`otlp_proto` encoding) and `otlp_logs_json`, `otlp_metrics_json`, `otlp_spans_json` (`otlp_json` encoding). A batch processor (`timeout: 200ms`, `send_batch_size: 100`) ensures data flows to Kafka in a steady stream rather than a single burst.
