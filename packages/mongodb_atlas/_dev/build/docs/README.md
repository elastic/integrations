...
- Collect MongoDB Atlas alert, mongod audit, mongod database, and organization logs, along with hardware and process metrics for comprehensive monitoring and analysis.
...
Logs help you keep a record of events that happen on your machine. The `Log` data stream collected by MongoDB Atlas integration are `alert`, `mongod_audit`, `mongod_database`, and `organization`.
