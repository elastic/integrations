...
The `alert` data stream collects alerts generated by the MongoDB Atlas. Alerts cover a wide range of logs and events, such as resource utilization thresholds (CPU, memory, disk space), database operations, security issues, and configuration changes. To collect alert logs, the requesting API Key must have the `Project Read Only` role.
